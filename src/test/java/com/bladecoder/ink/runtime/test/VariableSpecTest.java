...
public class VariableSpecTest {
    ...
    public void varList() throws Exception {
        List<String> text = new ArrayList<>();
        String json = TestUtils.getJsonString("inkfiles/test/test.ink.json");

        Story story = new Story(json);

        story.getVariablesState().set("meal", new InkList("MEAL_TYPE", story));

        //VariableAssignment var = new VariableAssignment("meal", false);
